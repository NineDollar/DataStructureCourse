...
BSTNode *BSTSearch(BSTNode *bt,KeyType k)	//在bt中查找关键字为k的结点
{
	BSTNode *p=bt;
	while (p!=NULL)
	{
...
int BSTInsert(BSTNode *&bt,KeyType k)	//在bt中插入关键字为k的结点
{	BSTNode *f,*p=bt;
...
void CreateBST(BSTNode *&bt,KeyType a[],int n)	//由a[0..n-1]创建bt
{
	bt=NULL;         			  //初始时bt为空树
	int i=0;
	while (i<n) 
...
void DestroyBST(BSTNode *&bt)	//销毁bt
{
	if (bt!=NULL)
	{
...
void DispBST(BSTNode *bt)		//采用括号表示输出bt
{
	if (bt!=NULL)
	{	
...
int BSTDelete(BSTNode *&bt,KeyType k)	//在bt中删除关键字为k的结点
{
